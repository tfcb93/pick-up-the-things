...
@export_range(30, 70, 1) var timer_collectable_probability := 0; ## Probability of a timer collectable (add 1 second on your timer) to appear in the field. I decided to close between 30 and 70 since I'm not shure about the probability distribution.
...
func add_to_collectables_list(area_x_0: float, area_x_1: float, area_z_0: float, area_z_1: float, sub_area_obstacle_pos: Vector3, total_area_collectables: int) -> void:
	...
	while(remaning_collectables_in_area > 0):
		...
		if (time_probability <= timer_collectable_probability and timer_collectables_generated < WorldGlobals.game_max_timer_collectables):
